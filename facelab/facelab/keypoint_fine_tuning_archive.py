...
import Progress_bar_extraction
import finetune_preparation
import train_model
...
class DraggablePointDialog(QtWidgets.QDialog):
    ...
    def initial_load(self):
        ...
        with h5py.File(os.path.join(Path(self.video_path).parent,"meta_key.h5"), 'r') as f:
            if self.event_frames:
                ...
                for all_event in self.event_frames:
                    temp_x.append(f['Facelab']["x"][all_event])
                    temp_y.append(f['Facelab']["y"][all_event])
                    self.temp_like.append(f['Facelab']["p"][all_event])

            else:
                print("No Event Information Loaded")
                temp_x = f['Facelab']["x"][:]
                temp_y = f['Facelab']["y"][:]
                self.temp_like = f['Facelab']["p"][:]
            paw_like_arr = [np.mean(paw_like) for paw_like in self.temp_like]
            self.min_likelihood_paw = np.where(np.array(paw_like_arr) <= 0.505)[0]
            if len(self.min_likelihood_paw)<1 :
                self.min_likelihood_paw = np.where(np.array(paw_like_arr) <= 0.53)[0]
                print("length is",len(self.min_likelihood_paw))


            print(max(np.diff(self.min_likelihood_paw)))
            max_diff=max(np.diff(self.min_likelihood_paw))
            if max_diff>50:
                temp_likelihood=[self.min_likelihood_paw[0]]
                for lp in range(1,len(self.min_likelihood_paw)):
                    if self.min_likelihood_paw[lp]-self.min_likelihood_paw[lp-1]>50:
                        temp_likelihood.extend([self.min_likelihood_paw[lp]])
                self.min_likelihood_paw=temp_likelihood
            else:
                self.min_likelihood_paw=self.min_likelihood_paw[::100]

            # if len(self.min_likelihood_paw)==0:
            #     self.min_likelihood_paw = np.where(np.array(paw_like_arr) <= 0.53)[0]

            if len(self.min_likelihood_paw)==0:
                raise Exception("There is no problem with the current Keypoints!")

            self.min_likelihood_paw=list(self.min_likelihood_paw)[:self.parent.frame_val]
        ...
        self.keypoint_coord.extend([[[temp_y[i][0],temp_x[i][0]],[temp_y[i][1],temp_x[i][1]],[temp_y[i][2],temp_x[i][2]]] for i in self.min_likelihood_paw])
        ...
        self.points.append(DraggablePointItem(pos=self.keypoint_coord[0][1], index=1, mydata=f"point1-{self.temp_like[self.min_likelihood_paw[self.curr_ind]][1]}",color=QtGui.QColor(self.colors[1]), parent=self))
        self.points.append(DraggablePointItem(pos=self.keypoint_coord[0][2], index=2, mydata=f"point2-{self.temp_like[self.min_likelihood_paw[self.curr_ind]][2]}",color=QtGui.QColor(self.colors[2]), parent=self))
    ...
    def update_key(self):
        ...
        for i in range(len(self.points)):
            ...
            self.points[i].dddata=f"{keylabels[i]}-{self.temp_like[self.min_likelihood_paw[self.curr_ind]][i]}"
            self.points[i].setData(pos=[self.keypoint_coord[self.curr_ind][i]], data=f"{keylabels[i]}-{self.temp_like[self.min_likelihood_paw[self.curr_ind]][i]}")
    ...
    def save_keypoint(self):
        ...
        # train_model.initiate_training(keypoint_saving_path,self.parent.lr_values ,destination_path=self.parent.destination_path,model_path=selected_model,parent=self,epoch=self.parent.epoch_val)
        # msg=QMessageBox()
